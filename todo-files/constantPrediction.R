...
constantPrediction.ClassifTask = function(task, measure, predict.type) {
  ...
  scores = vnapply(lvls, function(a) {
    # FIXME: If we use always a factor here, we can't use a measure that needs req.prob like auc
    arep = factor(rep(a, n), levels = lvls)
    data = data.frame(truth = y, response = arep)
    # we need to use task.desc from the task otherwise measures that need p$task.desc$positive such as tp or f1 wont work
