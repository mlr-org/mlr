context("oneclass_lofactor")

test_that("oneclass_lofactor", {
  requirePackagesOrSkip("DMwR", default.method = "load")
  ...
  for (i in seq_along(parset.list)) {
    ...
    # for lofactor, there is no seperation of test and train data
    # here: only predict on test, to compare with the restProbParsets fkt
    pars = list(data = oneclass.test[, -oneclass.col])
    pars = c(pars, parset)
    set.seed(getOption("mlr.debug.seed"))
    p = do.call(DMwR::lofactor, pars)
    old.probs.list[[i]] = convertingScoresToProbability(as.matrix(p))$probability[, 1]
  }

  testProbParsets("oneclass.lofactor", oneclass.df,
...
test_that("class names are integers and response predicted", {
  ...
  ae.lrn  = makeLearner("oneclass.lofactor", predict.type = "response", k = 20)
