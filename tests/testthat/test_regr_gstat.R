...
test_that("regr_gstat", {
  ...
  for (i in 1:length(parset.list)) {
    ...
    pars = list(formula = binaryclass.spatial.formula, data = binaryclass.spatial.train, locations = coordinates)
    pars = c(pars, parset)
    ...
    if (!is.null(pars$model.manual) || !is.null(pars$model.auto)) {
      ...
      # seeing an error here, even though we are passing data?
    ...
    p = predict(g, newdata = binaryclass.spatial.test)
    old.predicts.list[[i]] = p[,3]
  }
  testSimpleParsets(t.name = "regr.gstat", df = binaryclass.spatial, target = binaryclass.spatial.target,
    train.inds = binaryclass.spatial.train.inds, old.predicts.list = old.predicts.list, parset.list = parset.list)
})
