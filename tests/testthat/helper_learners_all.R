...
testThatLearnerRespectsWeights = function(lrn, task, train.inds, test.inds, weights, hyperpars) {
  ...
  expect_equal(getPredictionResponse(p1), getPredictionResponse(p2), info = lrn$id)
  expect_true(any(getPredictionResponse(p1) != getPredictionResponse(p3)), info = lrn$id)
}
