...
test_that("oversampling keeps all min / max obs", {
  y = binaryclass.df[, binaryclass.target]
  z = getMinMaxClass(y)
  new.inds = sampleBinaryClass(y, 1.05, cl = z$min.name, clreplace = TRUE, othreplace = FALSE)
  expect_true(setequal(intersect(z$min.inds, new.inds), z$min.inds))
  new.inds = sampleBinaryClass(y, 1.05, cl = z$max.name, clreplace = TRUE, othreplace = FALSE)
  expect_true(setequal(intersect(z$max.inds, new.inds), z$max.inds))
})
...
test_that("control which class gets over or under sampled", {
  ...
  rdesc = makeResampleDesc("CV", iters = 2)
  lrn1 = makeLearner("classif.rpart")
  lrn2 = makeUndersampleWrapper(lrn1, usw.rate = 0.1, cl = z$min.name)
  r = resample(lrn2, binaryclass.task, rdesc)
  expect_true(!is.na(r$aggr))
  lrn2 = makeOversampleWrapper(lrn1, osw.rate = 1.5, cl = z$max.name)
  r = resample(lrn2, binaryclass.task, rdesc)
  expect_true(!is.na(r$aggr))
})
