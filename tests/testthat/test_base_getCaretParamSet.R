...
test_that("getCaretParamSet", {
  ...
  caretLearners = c("gbm", "rf", "svmPoly", "svmLinear", "svmRadial",
    "rpart", "J48", "stepLDA", "earth")
  mycheck = function(lrn, k, x, y) {
    set.seed(123)
    a = capture.output({cps1 = getCaretParamSet(lrn, len = k, x = x, y = y, discretize = TRUE)})
    set.seed(123)
    b = capture.output({cps2 = getCaretParamSet(lrn, len = k, x = x, y = y, discretize = FALSE)})
  ...
  r1 = lapply(caretLearners, mycheck, k = 9, x = x, y = y)
  r2 = lapply(caretLearners, mycheck, k = 5, x = x, y = y)
  ...
  caretLearners = c("gbm", "rf", "svmPoly", "svmLinear",
    "rpart", "J48", "stepLDA", "earth")
  r1 = lapply(caretLearners, mycheck, k = 9, x = x, y = y)
  r2 = lapply(caretLearners, mycheck, k = 5, x = x, y = y)
})
