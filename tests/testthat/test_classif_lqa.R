...
test_that("classif_lqa", {
  ...
  old.probs.list[[1]] = predict(lqa(x, y, family = binomial(), penalty = lasso(0.01)), newx)$mu.new
  old.probs.list[[2]] = predict(lqa(x, y, family = binomial(), penalty = fused.lasso(c(0.001, 0.01))), newx)$mu.new
