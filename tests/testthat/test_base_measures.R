...
test_that("check measure calculations", {
  ...
  #jaccard
  jaccard.test = rowSums(cbind(tar1.multilabel, tar2.multilabel) * cbind(pred.multilabel$data[, 4L], pred.multilabel$data[, 5L])) / 
    (rowSums(cbind(tar1.multilabel, tar2.multilabel)) + rowSums(cbind(pred.multilabel$data[, 4L], pred.multilabel$data[, 5L])) - 
    rowSums(cbind(tar1.multilabel, tar2.multilabel) * cbind(pred.multilabel$data[, 4L], pred.multilabel$data[, 5L])))
  jaccard.test = ifelse(is.nan(jaccard.test), 1L, jaccard.test)
  jaccard.test = mean(jaccard.test)
