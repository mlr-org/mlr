...
test_that("1 numeric hyperparam", {
  ...
  expect_set_equal(
    sapply(plt$layers, function(x) class(x$geom)[1]),
...
test_that("1 discrete hyperparam", {
  ...
  orig = as.data.frame(res$opt.path)
  new = generateHyperParsEffectData(res, include.diagnostics = TRUE)
  new$data$kernel = factor(new$data$kernel,
    levels = c("vanilladot", "polydot", "rbfdot"))
  expect_equal(ignore_attr = TRUE, new$data, orig)
...
test_that("2 hyperparams", {
  ...
  res = tuneParams(learn,
    task = pid.task, control = ctrl, measures = acc,
    resampling = rdesc, par.set = ps, show.info = FALSE)
...
test_that("2 hyperparams nested", {
  ...
  suppressWarnings(expect_warning(print(plt)))
  dir = tempdir()
  path = file.path(dir, "test.svg")
  suppressWarnings(expect_warning(suppressMessages(ggsave(path))))
  expect_set_equal(
    sapply(plt$layers, function(x) class(x$geom)[1]),
