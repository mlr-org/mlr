...
# interpolating
ts.gstat = meuse.grid
ts.gstat$ts1 = sp::krige(log(zinc) ~ 1, meuse, meuse.grid, degree = 1)$var1.pred
ts.gstat$ts2 = sp::krige(log(zinc) ~ 1, meuse, meuse.grid, degree = 2)$var1.pred
ts.gstat$ts3 = sp::krige(log(zinc) ~ 1, meuse, meuse.grid, degree = 3)$var1.pred
# mapping
...
sp::ked.mlr.plot = sp::spplot(ked.mlr["response"], do.log = T, colorkey = TRUE, main = "log(zn) : ked interpolation (mlr)")
# SE - defining the standard error learner by altering the previous one.
se.lrn.krg = setPredictType(lrn.krg, predict.type = "se")
# training the SE model
se.mod.krg = train(se.lrn.krg, task.krg)
