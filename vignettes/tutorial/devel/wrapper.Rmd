...
* [Imputation](impute.html)
* [Bagging](bagging.html)
* [Tuning](tune.html)
* [Feature selection](feature_selection.html)
* [Cost-sensitive classification](cost_sensitive_classif.html)
* [Over- and undersampling](over_and_undersampling.html) for imbalanced classification problems
* Multiclass extension (`makeMulticlassWrapper()`) for binary-class learners
* [Multilabel classification](multilabel.html)
...
We choose to tune the parameters `minsplit` and `bw.feats` for the [mmce](measures.html) using a random search (`TuneControl()`) in a 3-fold CV:
...
3. The bagged models are evaluated using the mean [mmce](measures.html) (default aggregation for this performance measure) and new parameters are selected using the tuning method.
