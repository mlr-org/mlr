...
* Data preprocessing (`vignette("preproc")`)
* Imputation (`vignette("impute")`)
* Bagging (`vignette("bagging")`)
* Tuning (`vignette("tune")`)
* Feature selection (`vignette("feature_selection")`)
* Cost-sensitive classification (`vignette("cost_sensitive_classif")`)
* Over- and undersampling (`vignette("imbalanced_classification_problems")`) for imbalanced classification problems
* Multiclass extension (`makeMulticlassWrapper()`) for binary-class learners
* Multilabel classification (`vignette("multilabel")`)
...
We choose to tune the parameters `minsplit` and `bw.feats` for the `mmce` (`vignette("measures")`) using a random search (`TuneControl()`) in a 3-fold CV:
...
3. The bagged models are evaluated using the mean `mmce` (`vignette("measures")`) (default aggregation for this performance measure) and new parameters are selected using the tuning method.
