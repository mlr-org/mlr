...
If selected by the user they can be specified as explained on the tutorial page on [learners](learner.html) -- simply pass them to `makeLearner()`.
...
The evaluation method is already covered in detail in [evaluation of learning methods](performance.html) and [resampling](resample.html).
...
For this we need to create a resampling description just like in the [resampling](resample.html) part of the tutorial.
...
As no performance measure was specified, by default the error rate ([mmce](measures.html)) is used.
...
If you are interested in optimizing several measures simultaneously have a look at [Advanced Tuning](advanced_tune.html).

In the example below we calculate the accuracy ([acc](measures.html)) instead of the error rate. 
We use function `setAggregation()`, as described on the [resampling](resample.html) page, to additionally obtain the standard deviation of the accuracy. 
...
Note that we can also generate performance on the train data along with the validation/test data, as discussed on the [resampling](resample.html) tutorial page:
...
For an in-depth exploration of generating hyperparameter tuning effects and plotting the data, check out [Hyperparameter Tuning Effects](hyperpar_tuning_effects.html).
...
Have a look at `on.learner.error` in `configureMlr()` as well as the examples given in section [configure mlr](configureMlr.html) of this tutorial.
...
A clean approach to ensure unbiased performance estimation is [nested resampling](nested_resampling.html), where we embed the whole model selection process into an outer resampling loop.
