...
* Directly analyze the functional data using a [learner](learner.html) that is suitable for functional data on a [task](task.html). Those learners have the prefixes __classif.fda__ and __regr.fda__.
...
helper function `makeFunctionalData()` for the [fuelSubset](&fuelSubset.task)
...
For functional data, [learners](learner.html) are constructed using 
...
In this case, a normal regression- or classification-[learner](learner.html) 
can be applied.

This is explained in more detail in the [feature extraction](functional_data.html#feature-extraction)
section below.
...
In contrast to applying a learner that works on a [task](task.html) containing functional features,
the [task](task.html) can be converted to a normal [task](task.html).
