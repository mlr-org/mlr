...
All available `Task()`s are listed in the Appendix (`vignette("example_tasks")`).
...
Note, for later, that all standard resampling strategies (`vignette("resample")`) are supported.
...
Note, for later, that `mlr` offers much more functionality to deal with imbalanced classification problems (`vignette("over_and_undersampling")`).
