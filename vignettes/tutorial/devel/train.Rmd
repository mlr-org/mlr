...
All available `Task()`s are listed in the [Appendix](example_tasks.html).
...
Note, for later, that all standard [resampling strategies](resample.html) are supported.
...
Note, for later, that `mlr` offers much more functionality to deal with [imbalanced classification problems](over_and_undersampling.html).
