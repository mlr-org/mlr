...
Since no performance measure is specified the default for regression learners (mean squared error, [mse](measures.html)) is calculated.
...
Its name `"mse.test.mean"` indicates the performance measure, [mse](measures.html), and the method, `test.mean` (`aggregations()`), used to aggregate the 3 individual performances. 
...
Below, the error rate ([mmce](measures.html)), false positive and false negative rates ([fpr](measures.html), [fnr](measures.html)), and the time it takes to train the learner ([timetrain](measures.html)) are estimated by *subsampling* with 5 iterations.
...
(If you are interested in suppressing these messages permanently have a look at the tutorial page about [configuring mlr](configureMlr.html).)
...
Just as a `Prediction()` object (see the tutorial page on making [predictions](predict.html) it has an element `$data` which is a `data.frame` that contains the predictions and in the case of a supervised learning problem the true values of the target variable(s).
...
(For more detailed information on this topic see the [feature selection](feature_selection.html) page.)
...
One exception is the root mean square error ([rmse](measures.html)).
...
Below we calculate the mean misclassification error ([mmce](measures.html)) on the training
