...
When specifying the [task](task.html), you need to explicitly state that the task is `spatial` one by setting `spatial = TRUE`. 
...
The task (task.html) attribute `spatial` is set to `TRUE` to indicate that the data set also stores coordinate information.
