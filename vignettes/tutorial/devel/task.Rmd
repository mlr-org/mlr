...
* `ClassifTask()` for binary and [multi-class classification problems](cost_sensitive_classif.html) with class-dependent costs can be handled as well),
* `SurvTask()` for survival analysis,
* `ClusterTask()` for cluster analysis,
* `MultilabelTask()` for multilabel classification problems,
* `CostSensTask()` for general [cost sensitive classification](cost_sensitive_classif.html) (with example-specific costs).
...
The ID will be later used to name results, for example of [benchmark experiments](benchmark_experiments.html), and to annotate plots.
...
In binary classification the two classes are usually referred to as *positive* andc*negative* class with the positive class being the category of greater interest.
This is relevant for many [performance measures](performance.html) like the *true positive rate* or [ROC analysis](roc_analysis.html).
...
The names of the target columns are taken as class labels and need to be passed to the `target` argument of `makeMultilabelTask `Task()`.

In the following example we get the data of the yeast data set, extract the label names, and pass them to the ``target`` argument in `makeMultilabelTask `Task()`.
...
See also the tutorial page [multilabel](multilabel.html).
...
For more details see the page on [cost sensitive classification](cost_sensitive_classif.html).
...
This would indicate that some observations "belong together" and should not be separated when splitting the data into training and test sets for [resampling](resample.html).
...
See for example the tutorial page about [training](train.html) or function `makeWeightedClassesWrapper()`.
...
This especially comes in handy when you [integrate a new learner](create_learner.html) from another **R** package into `mlr`.
...
For more functions and more detailed explanations have a look at the [data preprocessing](preproc.html) page.
...
A list of all `Task()`s can be found in the [Appendix](example_tasks.html).
