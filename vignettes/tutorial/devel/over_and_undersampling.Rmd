...
As the *misclassification rate* ([mmce](measures.html){target="_blank"}) evaluates the overall
accuracy of the predictions, the *balanced error rate* ([ber](measures.html){target="_blank"}) and *area under the ROC Curve* ([auc](measures.html){target="_blank"}) might be more suitable here, as the misclassifications within each class are separately taken into account.

#### Over- and undersampling wrappers

Alternatively, `mlr` also offers the integration of over- and undersampling
via a [wrapper approach](wrapper.html){target="_blank"}. 
...
the [wrapper approach](wrapper.html){target="_blank"}.
