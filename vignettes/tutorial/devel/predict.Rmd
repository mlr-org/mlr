...
Just as `train()`, the `predict` (`predict.WrappedModel()`) function has a `subset` argument, so you can set aside different portions of the data in `Task()` for training and prediction (more advanced methods for splitting the data in train and test set are described in the section on [resampling](resample.html)).
...
If you are interested in tuning the threshold (vector) have a look at the section about [performance curves and threshold tuning](roc_analysis.html).
...
[mmce](measures.html) stands for *mean misclassification error*, i.e., the error rate.
See the sections on [performance](performance.html) and [resampling](resample.html) for further explanations.
