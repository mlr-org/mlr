...
Just as `train()`, the `predict` (`predict.WrappedModel()`) function has a `subset` argument, so you can set aside different portions of the data in `Task()` for training and prediction (more advanced methods for splitting the data in train and test set are described in the section on resampling (`vignette("resample")`)).
...
performance curves and threshold tuning (`vignette("roc_analysis")`).
...
`mmce` (`vignette("measures")`) stands for *mean misclassification error*, i.e., the error rate.
See the sections on performance (`vignette("performance")`) and
resampling (`vignette("resample")`) for further explanations.
