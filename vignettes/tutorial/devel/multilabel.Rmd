...
You can specify via the `measures` argument which [measure(s)](measures.html) to calculate.
The default measure for multilabel classification is the Hamming loss [multilabel.hamloss](measures.html).
All available measures for multilabel classification can be shown by `listMeasures()` and found in the table of performance [measures](measures.html) and the `?measures()` documentation page.
...
For evaluating the overall performance of the learning algorithm you can do some [resampling](resample.html). 
...
If you want to calculate a binary performance measure like, e.g., the  [accuracy](measures.html), the [mmce](measures.html) or the [auc](measures.html) for each label, you can use function `getMultilabelBinaryPerformances()`.
You can apply this function to any multilabel prediction, e.g., also on the resample multilabel prediction. 
For calculating the [auc](measures.html) you need predicted probabilities.
