...
A table showing all available methods can be found in `vignette("filter_methods")`.
...
In the following example we calculate the 10-fold cross-validated error rate `mmce` (`vignette("measures")`)) of the k-nearest neighbor classifier (`FNN::fnn()`) with preceding feature selection on the `iris` (`datasets::iris()`) data set.
...
If filtering is a preprocessing step before applying a learning method optimal values with regard to the learner performance can be found by tuning (`vignette("tune")`).
...
Here is another example using multi-criteria tuning (`vignette("tune")`).
...
During tuning both, the false positive and the false negative rate `fpr` (`vignette("measures")`) and `fnr` (`vignette("measures")`)), are minimized. As search strategy we choose a random search (see `makeTuneMultiCritControlRandom` (`?TuneMultiCritControl()`).
...
The performance is assessed by the holdout estimate of the concordance index `cindex` (`vignette("measures")`)).
...
In a second example we fit a simple linear regression model to the `BostonHousing` (`mlbench::BostonHousing()`) data set and use a sequential search to find a feature set that minimizes the mean squared error `mse` (`vignette("measures")`)).
