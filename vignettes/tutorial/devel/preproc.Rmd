...
Some of the following simple methods to change a `Task()` (or `data.frame`) were already mentioned on the page about learning tasks (`vignette("task")`):
...
* Feature selection (`vignette("feature_selection")`) and
* Imputation of missing values (`vignette("impute")`).
...
To this end `mlr` provides two wrappers (`vignette("wrapper")`):
...
* any parameters controlling the preprocessing as, e.g., the percentage of outliers to be removed can be tuned (`vignette("tune")`) together with the base learner parameters.
...
Below the performances of `MASS::qda()` with and without PCA preprocessing are compared in a benchmark experiment (`vignette("benchmark_experiments")`).
...
Those preprocessing and learner parameters can be tuned (`vignette("tune")`) jointly.
...
As described in the tutorial section about wrapped learners (`vignette("wrapper")`) wrappers are implemented using a *train* and a *predict* method.
...
Let's compare the cross-validated mean squared error (`mse` (`vignette("measures")`)) on the Boston Housing data set (`mlbench::BostonHousing()`) with and without scaling.
...
As already shown for the number of principal components in `makePreprocWrapperCaret()` we can tune (`vignette("tune")`) them easily together with other hyperparameters of the learner.
