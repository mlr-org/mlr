...
Some of the following simple methods to change a `Task()` (or `data.frame`) were already mentioned on the page about [learning tasks](task.html):
...
* [Feature selection](feature_selection.html) and
* [Imputation of missing values](impute.html).
...
To this end `mlr` provides two [wrappers](wrapper.html):
...
* any parameters controlling the preprocessing as, e.g., the percentage of outliers to be removed can be [tuned](tune.html) together with the base learner parameters.
...
Below the performances of `MASS::qda()` with and without PCA preprocessing are compared in a [benchmark experiment](benchmark_experiments.html).
...
Those preprocessing and learner parameters can be [tuned](tune.html) jointly.
...
As described in the tutorial section about [wrapped learners](wrapper.html) wrappers are implemented using a *train* and a *predict* method.
...
Let's compare the cross-validated mean squared error ([mse](measures.html)) on the Boston Housing data set (`mlbench::BostonHousing()`) with and without scaling.
...
As already shown for the number of principal components in `makePreprocWrapperCaret()` we can [tune](tune.html) them easily together with other hyperparameters of the learner.
