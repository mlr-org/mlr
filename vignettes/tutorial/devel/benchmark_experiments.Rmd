...
The entries show the [mean misclassification error](measures.html), the default performance measure for classification, on the test data set.
...
We use the mean misclassification error [mmce](measures.html) as primary performance measure, but also calculate the balanced error rate [ber](measures.html) and the training time `timetrain`.
...
The first plot shows the [mmce](measures.html) and the second plot the [ber](measures.html).
...
By default, the first measure in the `base::list()` of Measure's (`makeMeasure()`) passed to `benchmark()` is used, in our example [mmce](measures.html).
...
We choose the [mean misclassification error](measures.html).
...
Methods with best performance, i.e., with lowest [mmce](measures.html), are assigned the lowest rank.
...
See the tutorial page about [visualization](visualization.html) for details on data generation and plotting functions.
...
Below we generate grouped boxplots showing the error rate and the training time [timetrain](measures.html).
...
Below, function `GGally::ggpairs()` from package `GGally::GGally()` is used to generate a scatterplot matrix of [mean misclassification errors](measures.html) on the `mlbench::Sonar()` data set.
...
See for example the tutorial page on [ROC analysis](roc_analysis.html) and functions `generateThreshVsPerfData()`, `plotROCCurves()`, and `plotViperCharts()` as well as the page about [classifier calibration](classifier_calibration.html) and function `generateCalibrationData()`.
...
Reliable performance estimates can be obtained by [nested resampling](nested_resampling.html), i.e., by doing the tuning in an inner resampling loop while estimating the performance in an outer loop.
...
The general principle is explained in the section about [wrapper](wrapper.html) in the Advanced part of this tutorial. 
...
`mlr` offers some possibilities for [parallelization](parallelization.html).
