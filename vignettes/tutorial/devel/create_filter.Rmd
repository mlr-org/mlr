...
A lot of feature filter methods are already integrated in `mlr` and a complete list is given in the Appendix (`vignette("filter_methods")`) or can be obtained using `listFilterMethods()`.
...
`makeFilterWrapper()`; see also the page on feature selection (`vignette("feature_selection")`).
