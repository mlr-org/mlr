...
Below we compare the average costs and the error rate ([mmce](measures.html)) of the learning algorithm with both default thresholds 0.5 and theoretical thresholds.
...
The following plots show the cross-validated costs and error rate ([mmce](measures.html)).
...
list of [integrated learners](integrated_learners.html) in the Appendix or use `listLearners()`.
...
`mlr` provides several [wrappers](wrapper.html) to turn regular classification or regression methods into Learner (`makeLearner()`)s that can deal with example-dependent costs.
...
([meancosts](measures.html)) and the misclassification penalty ([mcp](measures.html)).
The latter measure is the average difference between the costs caused by the predicted class labels, i.e., [meancosts](measures.html), and the costs resulting from choosing the class with lowest cost for each observation.
