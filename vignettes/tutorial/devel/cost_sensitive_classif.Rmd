...
Below we compare the average costs and the error rate (`mmce` (`vignette("measures")`)) of the learning algorithm with both default thresholds 0.5 and theoretical thresholds.
...
The following plots show the cross-validated costs and error rate (`mmce` (`vignette("measures")`)).
...
list of integrated learners (`vignette("integrated_learners")`) in the Appendix or use `listLearners()`.
...
`mlr` provides several wrappers (`vignette("wrapper")`) to turn regular classification or regression methods into Learner (`makeLearner()`)s that can deal with example-dependent costs.
...
(meancosts (`vignette("measures")`)) and the misclassification penalty (mcp (`vignette("measures")`)).
The latter measure is the average difference between the costs caused by the predicted class labels, i.e., meancosts (`vignette("measures")`), and the costs resulting from choosing the class with lowest cost for each observation.
