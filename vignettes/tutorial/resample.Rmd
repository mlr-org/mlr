...
To initiate the grouping, we need to set `blocking.cv = TRUE` when creating the resample description object.
...
### Blocking with fully pre-defined indices

There is a second way of using pre-defined indices in resampling in `mlr`: Constructing the folds based on the supplied indices in `blocking`.
...
You can see that for every outer fold (List of 5), four inner folds were created that honor the grouping supplied via the `blocking` argument.
