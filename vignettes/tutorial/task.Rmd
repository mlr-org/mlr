...
The ID will be later used to name results, for example of `vignette("benchmark_experiments")`, and to annotate plots.
...
This would indicate that some observations "belong together" and should not be separated when splitting the data into training and test sets for `vignette("resampling")`.
...
See for example the tutorial page about `vignette("train")` or function `makeWeightedClassesWrapper()`.
...
This especially comes in handy when you [integrate a new learner](`vignette("create_learner")`) from another **R** package into `mlr`.
