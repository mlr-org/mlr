...
First, let's take a look at the effect for a classification task (`vignette("task")`).
...
As a result the performance (`vignette("performance")`) should improve if the model is applied to new data.
...
accuracy of the predictions, the *balanced error rate* ([ber](measures.md)) and *area under the ROC Curve* ([auc](measures.md)) might be more suitable here, as the misclassifications within each class are separately taken into account.

### Over- and undersampling wrappers

Alternatively, `mlr` also offers the integration of over- and undersampling
via a wrapper approach (`vignette("wrapper")`). 
This way over- and undersampling can be applied to already existing learners (`vignette("learners")`) to extend their functionality. 
...
the bagging approach (`vignette("bagging")`). 
...
Cost-Sensitive Classification (`vignette("cost_sensitive_classif")`)).
...
the wrapper approach (`vignette("wrapper")`).
