...
You can specify via the `measures` argument which measure(s) (`vignette("measures")`) to calculate.
The default measure for multilabel classification is the Hamming loss `multilabel.hamloss` (`vignette("measures")`)).
All available measures for multilabel classification can be shown by `listMeasures()` and found in the table of performance measures (`vignette("measures")`) and the `?measures()` documentation page.
...
For evaluating the overall performance of the learning algorithm you can do some resampling (`vignette("resampling")`). 
...
If you want to calculate a binary performance measure like, e.g., the  `accuracy` (`vignette("measures")`), the `mmce` (`vignette("measures")`) or the `auc` (`vignette("measures")`) for each label, you can use function `getMultilabelBinaryPerformances()`.
You can apply this function to any multilabel prediction, e.g., also on the resample multilabel prediction. 
For calculating the `auc`(`vignette("measures")`) you need predicted probabilities.
