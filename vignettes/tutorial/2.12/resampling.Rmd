...
<!-- ![Resampling Figure](pdf/img/resampling.png "Resampling Figure") -->
```{r echo=FALSE, out.width = "400pt", fig.align='center', fig.cap="Resampling Figure"}
knitr::include_graphics(here::here("vignettes/tutorial/2.12/pdf/img/resampling.png"))
```
...
Since no performance measure is specified the default for regression learners (mean squared error, `mse` (`vignette("measures")`)) is calculated.
...
Its name `"mse.test.mean"` indicates the performance measure, `mse` (`vignette("measures")`), and the method, `test.mean` (`aggregations()`), used to aggregate the 3 individual performances. 
...
Below, the error rate (`mmce` (`vignette("measures")`)), false positive and false negative rates (`fpr` (`vignette("measures")`), `fnr` (`vignette("measures")`)), and the time it takes to train the learner (`timetrain` (`vignette("measures")`)) are estimated by *subsampling* with 5 iterations.
...
(If you are interested in suppressing
these messages permanently have a look at the tutorial page about configuring mlr (`vignette("configureMlr")`).)
...
Just as a `Prediction()` object (see the tutorial page on making predictions (`vignette("predict")`)) it has an element `$data` which is a `data.frame` that contains the predictions and in the case of a supervised learning problem the true values of the target variable(s).
...
(For more detailed information on this topic see the feature selection (`vignette("feature_selection")`) page.)
...
One exception is the root mean square error (`rmse` (`vignette("measures")`)).
...
Below we calculate the mean misclassification error (`mmce` (`vignette("measures")`)) on the training
