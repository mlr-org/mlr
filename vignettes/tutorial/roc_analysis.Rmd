...
* determining an optimal decision threshold for given class prior probabilities and misclassification costs (for alternatives see also the pages about cost-sensitive classification (`vignette("cost_sensitive_classif")`) and imbalanced classification problems (`vignette("over_and_undersampling")`) in this tutorial),
...
`mlr` provides the AUC for binary classification (`auc` (`vignette("measures")`)) and also several generalizations of the AUC to the multi-class case (e.g., `multiclass.au1p` (`vignette("measures")`), `multiclass.au1u` (`vignette("measures")`) based on [Ferri et al. (2009)](https://www.math.ucdavis.edu/~saito/data/roc/ferri-class-perf-metrics.pdf)).

`mlr` offers three ways to plot ROC and other performance curves.

1. Function `plotROCCurves()` can, based on the output of `generateThreshVsPerfData()`, plot performance curves for any pair of performance measures (`vignette("measures")`) available in `mlr`.
...
Note that you can only use [earners (`vignette("learners")`) that are capable of predicting probabilities.
Have a look at the  learner table in the Appendix (`vignette("integrated_learners")`) or run `listLearners("classif", properties = c("twoclass", "prob"))` to get a list of all learners that support this.
...
Since we want to plot ROC curves we calculate the false and true positive rates (`fpr` (`vignette("measures")`)
and `tpr` (`vignette("measures")`)).
Additionally, we also compute error rates (`mmce` (`vignette("measures")`)).
...
The corresponding area under curve (`auc` (`vignette("measures")`)) can be calculated as usual by calling `performance()`.
...
tedious very quickly, we can use function `benchmark()` (see also Benchmark Experiments (`vignette("benchmark_experiments")`)) and, ideally, the support vector machine should have been tuned (`vignette("tune")`).
...
We assume that we are interested in a good performance over the complete threshold range and therefore tune with regard to the `auc` (`vignette("measures")`).
The error rate (`mmce` (`vignette("measures")`)) for a threshold value of 0.5 is reported as well.
...
As resampling strategy we use 5-fold cross-validation and again calculate the `auc` (`vignette("measures")`) as well as the error rate (for a threshold/cutoff value of 0.5).
