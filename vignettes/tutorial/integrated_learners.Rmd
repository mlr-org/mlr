...
See section cost-sensitive classification (`vignette("cost_sensitive_classif")`) and the documentation of `makeCostSensClassifWrapper()`, `makeCostSensRegrWrapper()` and `makeCostSensWeightedPairsWrapper()` for details.
...
See the documentation of function `makeMultilabelBinaryRelevanceWrapper()` and the tutorial section on multilabel classification (`vignette("multilabel")`) for details.
