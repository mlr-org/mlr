...
This could be either a performance measure which is not listed in the Appendix (`vignette("measure")`) or a measure that uses a misclassification cost matrix.

## Performance measures and aggregation schemes

Performance measures in `mlr` are objects of class `Measure` (`makeMeasure()`).
For example the mse (`vignette("measure")`) (mean squared error) looks as follows.
...
For plots there is also the option to use the longer measure `$name` instead. See the tutorial page on Visualization (`vignette("visualization")`) for more information.
...
and `$worst` and if it wants to be minimized or maximized (`$minimize`) during tuning (`vignette("tune")`)
or feature selection (`vignette("feature_selection")`).
...
See the tutorial page on resampling (`vignette("resampling")`) for some examples and the `?aggregations()` help page for all available aggregation schemes.
...
For in depth explanations and details see the tutorial page on cost-sensitive classification (`vignette("cost_sensitive_classif")`).
