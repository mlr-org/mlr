...
`ThreshVsPerfData` which contains the data for the plot in slot `$data`.
...
Generally, this can be done by manipulating the `ggplot2::ggplot()` object,
in this example the object returned by `plotThreshVsPerf()`, using the usual `ggplot2::ggplot2()` functions like `ggplot2::labs()` or `ggplot2::labeller()`.
...
`ggplot2::ggplot()` object (resulting from `plotThreshVsPerf()`, most often by
...
Using the `ggplot2::labeller()` function requires calling `ggplot2::facet_wrap()` (or `ggplot2::facet_grid()`), which can be
useful if you want to change how the panels are positioned (number of rows and columns) or influence the axis limits.
...
| generation function               | ggplot2 plotting function  | ggvis plotting function       | tutorial pages                                                                 |
 | --------------------------------  | -------------------------  | ----------------------------- | ------------------------------------------------------------------------------ |
 | `generateThreshVsPerfData()`      | `plotThresVsPerf()`        | `plotThreshVsPerfGGVIS()`     | `vignette("performance")`                                                      |
 |                                   | `plotROCCurves()`          | --                            | `vignette("roc_analysis")`                                                     |
 | `generateCritDifferencesData()`   | `plotCritDifferences()`    | --                            | `vignette("benchmark_experiments")`                                            |
 | `generateHyperParsEffectData()`   | `plotHyperParsEffect()`    |                               | `vignette("tune")`, `vignette("hyperparameter_tuning_effects")`                |
 | `generateFilterValuesData()`      | `plotFilterValues()`       | `plotFilterValuesGGVIS()`     | `vignette("feature_selection")`                                                |
 | `generateLearningCurveData()`     | `plotLearningCurve()`      | `plotLearningCurveGGVIS()`    | `vignette("learning_curve_analysis")`                                          |
 | `generatePartialDependenceData()` | `plotPartialDependence()`  | `plotPartialDependenceGGVIS()`| `vignette("partial_dependence")`                                               |
 | `generateFunctionalANOVAData()`   |                            |                               |                                                                                |
 | `generateCalibrationData()`       | `plotCalibration()`        | --                            | `vignette("classifier_calibration")`                                           |
