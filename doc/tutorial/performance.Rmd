...
To see which performance measures are implemented, have a look at [measures](http://www.statistik.tu-dortmund.de/~bischl/rdocs/mlr/html/measures.html). If you want 
...
the performance measures, the function [performance](http://www.statistik.tu-dortmund.de/~bischl/rdocs/mlr/html/performance.html) is used.
...
A nice method to calculate multiple performace measures at once makes use of the popular R function sapply. We define a list of measures and apply the performance function for each element.
...
probabilities are predicted, i.e. set the predict type of the [Learner](http://www.statistik.tu-dortmund.de/~bischl/rdocs/mlr/html/makeLearner.html) to "prob".
...
sapply(ms, function(meas) performance(pred, measure = meas))
```
