...
* `bw.replace` Sample bags with replacement? (Also known as *bootstrapping*)
* `bw.size` Percentage size of the sampled bags. (*Note: When `bw.replace=TRUE`, `bag.size=1` is default which does not mean, that one bag contains all the observations as observations will occur multiple times in each bag*)
* `bw.feats` Percentage size of randomy selected features for each iteration. 
