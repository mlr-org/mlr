...
task2 = makeMultilabelTask(id = "multi", data = iris[-c(1:10),], target = c("neu1","neu2"))
...
pr2 = predict(tr2,newdata=iris[1:10,-c(6,7)])
pr3 = predict(tr3,newdata=iris[1:10,])
pr3 = predict(tr3,newdata=iris[1:10,-c(6,7)])
...
r3 = resample(learner = lrn2, task = task2, resampling = rdesc, show.info = FALSE, measures = list(hamloss,mmce,acc,timetrain))
r2$pred$data$V1
...
task = makeClassifTask(id = "tutorial", data = iris[-c(1:10),], target = "neu1")
tr = train(lrn,task)
...
lrn3 = makeLearner("multilabel.rFerns", predict.type="prob")
tr3 = train(lrn3,task)
