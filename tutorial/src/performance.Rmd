...
## The default performance measure is the mean squared error (mse)
performance(pred)
## Calculate the median of squared errors (medse) instead
performance(pred, measures = medse)
```

Of course we can also calculate multiple performance measures at once by simply using a 
list of measures which can also include [your own measure](create_measure.md).

```{r}
## Calculate mean squared error, median squared error and mean absolute error
...
Note that for some performance measures it is required that you pass the [&Task] or the
[fitted model](&WrappedModel).

For example in order to assess the time needed for training, the fitted model has to be passed.
```{r}
...
Moreover, some measures may require a certain type of prediction.
For example in order to calculate the AUC ([&auc]) - the area under the ROC (receiver
...
The requirements of the individual measures are shown in the
[table of performance measures](&measures.md).

If you are interested in the properties of a single measure you can access them directly.
See the help page about [Measure](&makeMeasure) for information on the individual slots.
```{r}
## Mean misclassification error
str(mmce)
```

If you would like a list of available measures with certain properties use the function
[&listMeasures].
```{r}
