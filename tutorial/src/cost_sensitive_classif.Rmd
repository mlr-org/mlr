...
If the cost vector is given, the true class labels *y* are not needed to calculate the
expected costs.
The classification problem is therefore completely defined by the features *x* and the cost vector.
...
Now we fit a [logistic regression model](&nnet::multinom) to the
...
According to the cost matrix we should predict class Good only if we are very sure that Good
...
which requires the [ClassifTask](&Task) object and the cost matrix.
The cost matrix is passed via the argument ``costs``. It is expected that the rows indicate
...
We compare the average costs and the error rate ([mmce](measures.md)) of the learning algorithm
...
as well as other performance measures versus possible threshold values for the positive class.
...
Below we determine the threshold value for the positive class that leads to the
lowest average misclassification costs.
...
The tuned threshold for the positive class is close to the theoretical threshold
...
Tuning the threshold on the complete training data set as shown above can lead to overfitting.
To avoid this resampling strategies should be used.
Below we perform 3-fold cross-validation and use [&tuneThreshold] to find threshold values
with lowest average costs over the 3 test data sets.

```{r}
## 3-fold cross-validation
rdesc = makeResampleDesc("CV", iters = 3)
r = resample(lrn, credit.task, resampling = rdesc, measures = list(credit.costs, mmce), show.info = FALSE)
r
...
In the following we compare the cross-validated performance of the tuned and the theoretical
threshold.
...
performance(setThreshold(r$pred, th), measures = list(credit.costs, mmce))
```
...
If additionally our target threshold *t* is the theoretical threshold *t*<sup>*</sup> the
proportion of observations in the positive class has to be multiplied by
...
Naturally, this is only possible for methods with either a class weights or observation weights argument.
...
In this case you have to additionally pass the name of the relevant learner parameter (``"class.weights"``)
...
For this purpose we have to create a wrapped [Learner](&makeLearner) via function
[makeOversampleWrapper](&makeUndersampleWrapper) and tune the parameter `osw.rate` using
function [&tuneParams].
...
```{r}
## Threshold tuning based on 3-fold cross-validated predictions
rdesc = makeResampleDesc("CV", iters = 3)
r = resample(lrn, wf.task, resampling = rdesc, measures = list(wf.costs, mmce), show.info = FALSE)
r
...
vector chosen above as well as its cross-validated performance.

```{r}
th/sum(th)
performance(setThreshold(r$pred, th), measures = list(wf.costs, mmce))
```
