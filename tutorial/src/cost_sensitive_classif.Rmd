...
[%mlr] provides several [wrappers](wrapper.md) to create [Learner](&makeLearner)s that can
...
In the following example we calculate the mean costs of the predicted class labels ([meancosts](&measures)) and
the misclassification penalty ([mcp](&measures)).
The latter measure is the average difference between the costs caused by the predicted
class labels, i.e., [meancosts](&measures), and the costs resulting from choosing the class with lowest
