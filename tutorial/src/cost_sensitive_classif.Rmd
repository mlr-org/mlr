...
elements *c*(2,1) = *c*(3,1) = 50, *c*(1,2) = *c*(3,2) = 4.5 and *c*(1,3) = *c*(2,3) = 9.
...
Similar accessor functions ([&getCostSensClassifModel] and [&getCostSensRegrModels]) exist
for the other two approaches.

```{r}
getCostSensWeightedPairsModels(mod)
```
...
In order to compute the misclassification penalty the costs for the test observations are
required and therefore the [&Task] has to be passed to [&performance].
