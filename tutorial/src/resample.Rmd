...
set.
The default performance measure in survival analysis is the concordance index ([cindex](&measures)).
...
In the above example the performance measure is the concordance index ([cindex](&measures)).
...
In the following we estimate the Dunn index ([dunn](&measures)), the Davies-Bouldin cluster
separation measure ([db](&measures)), and the time for training the learner ([timetrain](&measures))
...
By default, the resulting [&makeWrappedModel]s are not returned by [&resample].
