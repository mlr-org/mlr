...
r = resample(learner = lrn, task = sonar.task, resampling = rin, show.info = FALSE)
r$aggr
```
And now with bagging:
```{r makeBaggingWrapper_with}
rin = makeResamplingDesc("CV", iters = 10)
result = resample(learner = bag.lrn, task = sonar.task, resampling = rin, show.info = FALSE)
result$aggr
```
...
With the function [&getBaggingModels], you can access the models fitted in the individual iterations.
```{r}
head(getBaggingModels(mod), 2)
```
