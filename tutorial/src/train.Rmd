...
with a learner and a suitable [&Task].
...
calling the function [&train], passing it a [&Learner] and a [&Task].
In the examples above, the machine learning method was specified
through special [Learner][&makeLearner] objects. Creating a separate object
...
The return value is always an object of class [&WrappedModel], which wraps the
particular model of the used R classification or regression method. It
can subsequently be used to perform a [prediction][&predict.WrappedModel] for new observations.
