...
[Cox proportional hazards model](&survival::coxph) on the [lung](&survival::lung) data set.
...
In the above example, creating a [Learner](&makeLearner) is not strictly necessary.
Generally, you have to generate a [Learner](&makeLearner) if you want to change any defaults
...
Training a learning method works the same for every type of [&Task].
Here is a cluster analysis example using the data set [mtcars](&dataset::mtcars).
Since we want to set a hyperparameter value we have to create a [Learner](&makeLearner).
...
The object [bh.task](&mlr.bh) is the regression [&Task] on the [BostonHousing](&mlbench::BostonHousing)
...
As you may recall, it is also possible to specify observation weights when creating the 
[&Task].
...
[&train] returns an object of class [&WrappedModel], which wraps the
particular model (i.e., contains the output) of the underlying **R** learning method.
