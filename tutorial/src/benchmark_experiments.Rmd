...
The entries show the 5-fold cross-validated mean misclassification error ([mmce](measures.md)), which
is the default performance measure for classification.

The result is an object of class [&BenchmarkResult].
[%mlr] provides several accessor functions, follow the naming convention
``getBMR<what_to_extract>``, that allow to extract e.g. the performances or the predictions
...
Note that in the examples above the task and learner IDs are used to name the list elements.
The IDs can be accessed as follows:
...
Often it more convenient to work with data frames, so you can easily convert the result structure:
