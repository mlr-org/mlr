...
Columns **Fac.** and **Num.** show if a particular importance measure can deal with factor and
numeric features.
...
filtered.task = filterFeatures(iris.task, fval = fv, select = "perc", val = 0.25)

## Keep all features with importance greater than 0.5
filtered.task = filterFeatures(iris.task, fval = fv, select = "threshold",
    val = 0.5)
filtered.task
```

You can either specify the ``method`` for calculating the feature importance or you can
...
In order to access the selected feature subsets we can call [&getFilterResult] on each model.

```{r}
sfeats = sapply(r$models, getFilterResult)
table(sfeats)
```
