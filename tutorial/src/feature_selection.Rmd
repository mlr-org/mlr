...
Columns **Fac.** and **Num.** show if a particular importance measure can deal with numeric and
factor features.
...
As mentioned above the feature importance can be calculated by function [&getFilterValues] 
given the [&Task] and a character string specifying the filter method. 

```{r}
fv = getFilterValues(iris.task, method = "information.gain")
fv
```

``fv`` is a [&FilterValues] object and ``fv$data`` contains a ``data.frame`` that gives the
importance values for all features.
...
* Keep a certain *absolute number* of features with highest importance.
* Keep a certain *percentage* of features with highest importance. 
...
filtered.task = filterFeatures(iris.task, method = "information.gain", select = "abs", val = 2)

## Keep the 25% most important features
filtered.task = filterFeatures(iris.task, method = "information.gain", select = "perc", val = 0.25)

## Keep all features with importance greater than 0.5
filtered.task = filterFeatures(iris.task, method = "information.gain", select = "threshold", 
    val = 0.5)
filtered.task
```
...
In the following example we calculate the 10-fold cross-validated error rate ([&mmce]) of the 
[k nearest neighbor classifier](&FNN::fnn) with preceding feature selection on the 
...
```{r}
lrn = makeFilterWrapper(learner = "classif.fnn", fw.method = "information.gain", 
    fw.select = "abs", fw.val = 2)
...
```{r}
# sapply(r$models, getFilterResult)
sfeats = sapply(r$models, getFilteredFeatures)
table(sfeats)
```

FIXME difference between getFilterResult and getFilteredFeatures? [FilterResult](&getFilterResult)
...
FIXME also show benchmark experiment, train?
...
such that the 3-fold cross-validated mean squared error (see [bh.rin](&mlr.bh) and [&mse]) 
...
lrn = makeFilterWrapper(learner = "regr.lm", fw.method = "chi.squared", 
    fw.select = "perc")
ps = makeParamSet(makeDiscreteParam("fw.val", values = seq(0.2, 0.5, 0.05)))
res = tuneParams(lrn, task = bh.task, resampling = bh.rin, par.set = ps, 
...
```{r}
lrn = makeFilterWrapper(learner = "regr.lm", fw.method = "chi.squared", 
    fw.select = "threshold", fw.val = res$x$fw.val)
...
During tuning both, the false positive and the false negative rate ([&fpr] and [&fnr]), are
minimized. As search strategy we choose a random search 
(see [makeTuneMultiCritControlRandom](&TuneMultiCritControl)).

```{r}
lrn = makeFilterWrapper(learner = "classif.lda", fw.method = "chi.squared", 
    fw.select = "threshold")
ps = makeParamSet(makeNumericParam("fw.val", lower = 0.1, upper = 0.9))
res = tuneParamsMultiCrit(lrn, task = sonar.task, resampling = sonar.rin, par.set = ps, 
...
The performance is assessed by the holdout estimate of the concordance index ([&cindex]).
...
## Select features
sfeats = selectFeatures(learner = "regr.lm", task = bh.task, resampling = bh.rin, control = ctrl, 
    show.info = FALSE)
sfeats
```
...
lrn = makeFeatSelWrapper("surv.coxph", resampling = rdesc, 
    control = makeFeatSelControlRandom(maxit = 10L), show.info = FALSE)
