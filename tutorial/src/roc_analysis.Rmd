...
misclassification costs (that would allow to calculate an optimal threshold value) are unknown. 
...
generalization of the AUC for the multi-class case ([&muticlass.auc]) from package
[pROC](&http://cran.r-project.org/web/packages/pROC/index.html).
...
calculate the [performance](&ROCR::performance) measures and plot the ROC curve.
...
Otherwise, [&ROCR::plot.performance] will draw one curve for each fold.
For details, see [&ROCR::plot.performance].
