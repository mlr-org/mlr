...
1. [%mlr]'s function [&plotROCRCurves] is a convenient interface to [%ROCR]'s
   [plot](&ROCR::plot-methods) function.
...
## Performance plots with plotROCRCurves

As mentioned above [&plotROCRCurves] is an interface to [%ROCR]'s
[plot](&ROCR::plot-methods) function.
...
Per default, [&plotROCRCurves] draws a ROC curve and adds a diagonal line that represents
the performance of a random classifier.

```{r}
plotROCRCurves(pred1, avg = "none")
```
...
Per default a [legend](&graphics::legend) is added and the names of the list elements are used
for annotation.
Changes to the [legend](&graphics::legend) can be made through argument `legend.args` of
[&plotROCRCurves].

```{r}
plotROCRCurves(list(lda = pred1, ksvm = pred2), avg = "none")
```
...
```{r}
plotROCRCurves(list(lda = pred1, ksvm = pred2), meas1 = "lift", meas2 = "rpp", avg = "none",
  add.diag = FALSE, legend.args = list(x = "bottomleft"))
```
...
```{r}
plotROCRCurves(list(lda = pred1, ksvm = pred2), meas1 = "acc", meas2 = "cutoff", avg = "none",
  add.diag = FALSE, legend.args = list(x = "bottom"))
```
...
For more averaging methods and details see [ROCR's plot function](&ROCR::plot-methods).

If you want to plot the individual ROC curves for each resample iteration set `avg = "none"`.

```{r}
plotROCRCurves(res, avg = "none")
```
...
## Draw ROC curves
plotROCRCurves(preds2, avg = "none")
```
...
3. [ROCR::plot](&ROCR::plot-methods): Plot a performance measure either versus another measure
   or versus the threshold/cutoff.
...
[%ROCR]-based code or other packages that depend on [%ROCR].
