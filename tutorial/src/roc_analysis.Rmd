...
1. [%mlr]'s function [&plotROCRCurves] is a convenient interface to [%ROCR]'s
   [performance](&ROCR::performance) methods with a [%ggplot2] output.
...
```{r}
plotROCRCurves(pred1, diagonal = TRUE)
```
...
```{r}
plotROCRCurves(list(lda = pred1, ksvm = pred2))
```
...
```{r}
plotROCRCurves(list(lda = pred1, ksvm = pred2), meas1 = "lift", meas2 = "rpp")
```
...
```{r}
plotROCRCurves(list(lda = pred1, ksvm = pred2), meas1 = "acc", meas2 = "cutoff")
```
