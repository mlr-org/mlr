...
values.
[%mlr] provides the [auc](measures.md) performance measure for binary classification and also a
...
calling [predict](&predict.WrappedModel), [&resample], or [&benchmark]) to a format that
[%ROCR] can handle.
Then functions in [%ROCR] are used
...
We use the [Sonar](&mlbench::Sonar) data set from package
[%mlbench].
...
rdesc = makeResampleInstance(rdesc, sonar.task)

## Calculate the error rate (for threshold 0.5) and the AUC
ms = list(mmce, auc)
r1 = resample(lrn1, sonar.task, rdesc, measures = ms, show.info = FALSE)
r2 = resample(lrn2, sonar.task, rdesc, measures = ms, show.info = FALSE)
```
...
[%mlr] also supports [ViperCharts](http://viper.ijs.si/) for plotting ROC
curves. In this case, we can use the predictions directly.
```{r}
z = plotViperCharts(list(lda=r1$pred, ksvm=r2$pred), chart = "rocc", browse = FALSE)
```
You can see the plot created this way [here](`r z`). For details, see [&plotViperCharts].
