...
Furthermore, [%mlr] permits to [create your own imputation method](create_imputation.md).

Note that some of the learning algorithms included in [%mlr] can deal with missing values in a sensible way, i.e.,
other than deleting observations with ``NA's``.
Those [Learner](&makeLearner)s have the property ``"missings"`` and are indicated in the
[table of integrated learners](integrated_learners.md).
...
For ``impute.new.levels`` and ``recode.factor.levels`` see the help page for function [&impute].
...
```{r}
lrn = makeImputeWrapper(makeLearner("regr.lm"), cols = list(Solar.R = imputeHist(),
  Wind = imputeLearner(makeLearner("classif.rpart"))), dummy.cols = c("Solar.R", "Wind"))
lrn
```
...
In the following the 3-fold cross-validated [mean squared error](&measures) is calculated.
