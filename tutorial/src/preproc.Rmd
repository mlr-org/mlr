...
* Some simple options to modify a [&Task] described in section [Learning tasks](task.md)
* [Feature selection](feature_selection.md)
* [Imputation of missing values](impute.md)

Moreover, [%mlr] permits to fuse a [Learner](&makeLearner) with any preprocessing method of
...
can be [tuned](tune.md) alongside the base learner parameters.


## Fusing a learner with preprocessing
...
and prediction and are completely defined by the user.
...
In the following we show how to couple a [Learner](&makeLearner) with function
[scale](&base::scale).
