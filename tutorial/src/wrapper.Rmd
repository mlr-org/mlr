...
```
We choose to tune the parameters `minsplit` and `bw.feats` for the [mmce](&measures) using a [random search](&TuneControl) in a 3-fold CV:
```{r}
...
3. The bagged models are evaluated using the mean [mmce](&measures) (default aggregation for this performance measure) and new parameters are selected using the tuning method.
