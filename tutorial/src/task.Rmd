...
In case of supervised learning like regression (as well as classification and
survival analysis) we, additional to ``data``, have to specify the name of the numeric ``target``
variable.
...
For each of these learning problems there are some specifics worth mentioning and described
in the following.


### Classification tasks

In case of classification the target variable has to be a ``factor``.
...
In case of survival analysis ``target`` specifies the names of the survival time and event
...
In *cost-sensitive classification* the costs associated with assigning an object with true
class *y* to class *k* can be different depending on the true and predicted class labels
*y* and *k*.
...
Thus, in case of a *K*-class problem, each example (*x*, *y*) is associated with a vector
of costs for predicting classes 1,...,*K* given the true class label *y*.
The cost of the true class label *y* is minimal.
Therefore, if the cost matrix is given, the true values *y* of the target variable are 
automatically known.

Thus, in order to create a cost-sensitive classification task, the feature values *x* and
...
In Machine Learning: Proceedings of the 22rd International Conference, pp. 49–56. ACM, 2005.

For more details see the section about [cost-sensitive classification](&cost_sensitive_classification.md).
...
[&Task] is a ``list`` and the main properties of the learning task are stored in element
