...
In the following example we calculate the accuracy ([acc](&measures)) instead of the error
rate.
...
After tuning you can generate a [Learner](&makeLearner) with optimal hyperparameter values
as follows:
...
During tuning you might want to optimize multiple, maybe conflicting, performance measures
simultaneously.
