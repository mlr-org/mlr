...
`par.val` argument in the [&makeLearner] method.
...
We need to create a [ParamSet](&makeParamSet) object that describes
the parameter space we wish to search. This is done through the function
[&makeParamSet]. We add discrete parameters for the `C` and `sigma` parameter
...
using our [&TuneControl] instance and the resampling strategy
described by the `rdesc` variable.
We use the [iris.task](&mlr.iris) provided by **mlr**.

```{r}
res = tuneParams("classif.ksvm", task = iris.task, resampling = rdesc, par.set = ps, 
    control = ctrl, measures = list(mmce, setAggregation(mmce, test.sd)))
```

We used a trick, also described in section [Resampling](&resample.md), to
obtain the standard deviation in addition to the default by adding a second
measure. A quick visualization of the search can be achieved by accessing
...
A good approach for getting a better feeling for the best parameter setting
is to do a nested cross-validation.
...
```{r}
r = resample(lrn, iris.task, resampling = out.rdesc, extract = getTuneResult, show.info = FALSE)
```
...
Let's tune `k` of a [k-nearest neighbour regression model](&kknn::kknn) (implemented
in package [kknn](http://www.rdocumentation.org/packages/kknn)) on the 
[BostonHousing](&mlbench::BostonHousing) data set.
...
tuneParams(learner = "regr.kknn", task = bh.task, resampling = rdesc, par.set = ps, 
    control = ctrl, measures = mse)
```
