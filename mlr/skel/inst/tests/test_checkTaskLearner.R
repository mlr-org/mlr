...
test_that("checkTaskLearner", {
  ...
  expect_error(train(makeLearner("classif.lda"), task), "missing values, but")
  expect_error(train(makeLearner("regr.km"), regr.task), "factor inputs, but")
  expect_error(train(makeLearner("classif.gbm"), multiclass.task), "multiclass-problem, but")
