...
#'
#'   return(semimetric.mlr.matrix(fdata1 = fd1$data, fdata2 = fd2$data,
...
#'   # the distances are computed in semimetric.mlr.matrix.
...
#'   # semimetric.mlr.matrix can be used with evenly spaced data that does not need
#'   # derivation.
#'   if(evenly.spaced & (nderiv == 0L | derived)) {
#'     return(semimetric.mlr.matrix(fdata1 = fdata1$data, fdata2 = fdata2$data,
#'                                  nderiv = nderiv, method = method,
#'                                  evenly.spaced = TRUE, derived = derived,
#'                                  ...))
#'   } else {
...
#'     stopifnot(dmin >= 1L)
#'     stopifnot(dmax <= ncol(mat1))
#'     computeSemimetric(mat1[,dmin:dmax], mat2[,dmin:dmax], "Euclidean")
#'   }
#'
#'   if(method == "mean") {
#'     return(outer(rowMeans(mat1), rowMeans(mat2), "-"))
#'   }
#'
#'   if(method == "relAreas") {
#'     stopifnot(dmin >= 1L)
