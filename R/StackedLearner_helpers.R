...
doTrainPredict = function(bl, task, show.info, id, save.on.disc) {
  ...
  return(list(base.models = model, pred = pred))
}
...
aggregateModelPredictions = function(.model, pred.list, weights = NULL) {
  ...
  if (.model$type == "classif") {
    ...
    if (task.desc$type == "regr") {
      y = apply(simplify2array(preds), 1, weighted.mean, w = weights, na.rm = TRUE)
    } else if (task.desc$type == "classif") {
      y = apply(simplify2array(preds), c(1, 2), weighted.mean, w = weights, na.rm = TRUE)
    }
  }
