...
#'   If `NULL`, the rate will be calculated so that the minorty class(es) get the same size as the majority class.
...
smote = function(task, rate = NULL, nn = 5L, standardize = TRUE, alt.logic = FALSE) {
  checkTask(task)
  ...
  if (any(z$min.sizes < nn))
    stopf("You cannot set nn = %i, when the minimal class has size %i!", nn, z$min.sizes)

  res = lapply(z$min.names, function(s) {
    x.min = x[z$min.inds[[s]], , drop = FALSE]
    n.min = nrow(x.min) # number of NEW cases
    if (is.null(rate)) {
      r = z$max.size/n.min
    }
    n.new = ifelse(alt.logic, as.integer(r - 1) * n.min, round((r - 1) * n.min))
    if (n.new <= 0L)
      return(data.frame())
  ...
  data2 = rbind(data, as.data.frame(res))
