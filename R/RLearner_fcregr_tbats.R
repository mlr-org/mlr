...
makeRLearner.fcregr.tbats = function() {
  makeRLearnerForecastRegr(
    ...
    par.set = makeParamSet(
      ...
      makeLogicalLearnerParam(id = "use.parallel",# default = expression(length(y) > 1000),
                              tunable = FALSE),
      ...
      # prediction
      makeIntegerLearnerParam(id = "h", lower = 0, upper = Inf,# default = expression(ifelse(object$arma[5] > 1, 2 * object$arma[5], 10)),
                              tunable = FALSE,
                              when = "predict"),
