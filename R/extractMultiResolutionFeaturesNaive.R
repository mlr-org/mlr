...
# @description
# extractFDAMultiResFeatures differs from getUniFDAMultiResFeatures in that it accept custom segment
# input, if the user does not provide this parameter then by default it extracts on the whole curve
# note that curve.lens is just some user defined segments in one channel!
getFDAMultiResFeatures = function(data, target, cols, res.level = 3L, shift = 0.5, curve.lens = NULL) {
...
getUniFDAMultiResFeatures = function(data, res.level = 3L, shift = 0.5) {
  ...
  # FIXME: Maybe do this in Cpp aswell
