...
#'
#' @param level1Learner The learner which produces predicted labels at prediction time. Default is \code{learner}, i.e. the same learner as the passed one.  
...
makeMultilabelStackingWrapper = function(learner, level1Learner = learner, cv.folds = 2) {
...
trainLearner.MultilabelStackingWrapper = function(.learner, .task, .subset, .weights = NULL,...) {
  ...
  h = function(tn) {
    ...
    train(.learner$level1Learner, ctask, weights = .weights)
  }
  modelsLvl1 = lapply(targets, h)
  # predict labels
  f = function(tn) {
    data2 = dropNamed(data, setdiff(targets, tn))
    ctask = makeClassifTask(id = tn, data = data2, target = tn)
    r = resample(.learner$level1Learner, ctask, weights = .weights,
          makeResampleDesc("CV", iters = .learner$cv.folds), show.info = FALSE)
    as.numeric(as.logical(r$pred$data[order(r$pred$data$id), ]$response))
  }
...
predictLearner.MultilabelStackingWrapper = function(.learner, .model, .newdata, ...) {
  ...
  if (.learner$level1Learner$predict.type == "response") {
  ...
  g = if (.learner$predict.type == "response") {
    ...
    function(m) getPredictionProbabilities(predict(m, newdata = nd, ...))
  }
