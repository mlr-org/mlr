...
calculateResampleIterationResult = function(learner, task, train.i, test.i, measures,
  ...
  if (pp == "train") {
    ...
    ms.train = performance(task = task, model = m, pred = pred.train, measures = measures, train = train.i)
    ...
    ms.test = performance(task = task, model = m, pred = pred.test, measures = measures, train = train.i)
