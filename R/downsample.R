...
#' @export
downsample = function(obj, target, percentage = NULL, n = NULL, stratify = FALSE) {
  checkArg(obj, c("data.frame", "SupervisedTask", "ResampleInstance"))
  checkDownsampleArguments(percentage, n, stratify)
  UseMethod("downsample")
}

#' @export
downsample.data.frame = function(obj, target = NULL, percentage = NULL, n = NULL, stratify = FALSE) {
  if(!isNotSet(target)) {
  ...
  if(isNotSet(target)) {
    if(is.null(percentage)) {
      percentage = n / nrow(obj)
    }
    holdoutDesc = makeResampleDesc(method = "Holdout", stratify = stratify, split = percentage)
    ...
    tsk = downsample(tsk, percentage = percentage, n = n, stratify = stratify)
    res = getTaskData(tsk)
  }
  res
}
...
#' @export
downsample.SupervisedTask = function(obj, target, percentage = NULL, n = NULL, stratify = FALSE) {
  if(is.null(percentage)) {
    percentage = n / obj$task.desc$size
  }
  holdoutDesc = makeResampleDesc(method = "Holdout", stratify = stratify, split = percentage)
...
#' @export
downsample.ResampleInstance = function(obj, target, percentage = NULL, n = NULL, stratify = FALSE) {
  if(stratify) {
    stop("Stratifying is not supported for a ResampleInstance!")
  }
  obj$train.inds = lapply(obj$train.inds, function(x) {
    if(is.null(n)) {
      n = round(percentage * length(x))
    }
    if(n > length(x)) {
...
checkDownsampleArguments = function(percentage = NULL, n = NULL, stratify = FALSE) {
