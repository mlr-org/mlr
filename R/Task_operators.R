...
changeData = function(task, data, costs, weights, labels = NULL) {
  ...
  task$task.desc = switch(td$type,
    "classif" = makeClassifTaskDesc(td$id, data, labels, task$weights, task$blocking, td$positive, td$is.spatial),
    "regr" = makeRegrTaskDesc(td$id, data, labels, task$weights, task$blocking, td$is.spatial),
    "cluster" = makeClusterTaskDesc(td$id, data, task$weights, task$blocking, td$is.spatial),
    "surv" = makeSurvTaskDesc(td$id, data, labels, task$weights, task$blocking, td$is.spatial),
    "costsens" = makeCostSensTaskDesc(td$id, data, labels, task$blocking, costs, td$is.spatial),
    "multilabel" = makeMultilabelTaskDesc(td$id, data, labels, task$weights, task$blocking, td$is.spatial)
  )
