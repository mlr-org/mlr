...
#'   by crossvalidated predictions (the internal resampling strategy can be set via the `resampling` argument).}
#'   \item{\code{ensembleselection}}{Select a subset of base learner predictions by ensenemble selection}
#'  }
#'
#' @param id (`character(1)`)`  Id string for object. Used to display object
#' and for model saving. Default is "stack".
#' @param method (`character(1)`)\cr
#' \describe{
#'  \item{`aggregate`}{Averaging of base learner predictions without weights.}
#'  \item{`superlearner`}{For building a super learner using crossvalidated predictions of the base learners.}
#'  \item{`ensembleselection`}{For averaging the (cross validated) predictions of the base learners, with the weights learned from
#'   ensemble selection algorithm.}
#' }
#' @param base.learners (list of [Learner])\cr
#'   A list of learners created with ([makeLearner]). The prediction type can
#'   be changed using `setPredictType`.
#' @param super.learner learner ([Learner] | `character(1)`)\cr
...
#'   created via `makeLearner`. Only used for `method = 'superlearner'`.
#'   Default is `NULL`.
#' @param predict.type (`character(1)`)\cr
#'   Sets the type of the final prediction.
#'   For method `super.learner` the predict type can also be set within `super.learner`.
#'   If the type of the base learner prediction, which is set up within `base.learners`, is
#'   \describe{
#'    \item{`prob`}{`predict.type = 'prob'` will use the aggregate of all
#'    base learner predictions and `predict.type = 'response'` will use
#'    the class with highest probability as final prediction.}
#'    \item{`response`}{For classification tasks with `predict.type = 'prob'`,
...
#'   Resampling strategy for `method = 'superlearner'` and `method = 'ensembleselection'`.
#'   Only CV is allowed for resampling. The default `NULL` uses 5-fold CV.
#' @param use.feat (`logical(1)`)\cr
#'   Whether the original features should also be passed to the super learner.
#'   Only used for `method = 'superlearner'`. Default is `FALSE`.
...
#'   \item{`tolerance`}{Minimum improvement in ensemble performance in order to continue adding learners.}
#' }
#' @param save.on.disc (`logical(1)`)\cr
#'   If set to `TRUE`, base models are saved on disc at the working directory.
...
#'   If set to `FALSE` models will not contain predictions. This reduce the
#'   object size. Note that function `resampleStackedLearnerAgain` does not
#'   work if saving prediction is disabled. Default is `TRUE`.
