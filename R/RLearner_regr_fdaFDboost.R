...
trainLearner.regr.fdaFDboost = function(.learner, .task, .subset, .weights = NULL, mstop = 100L,
  ...
  FDboost::FDboost(formula = form, timeformula = ~bols(1), data = mat.list, control = ctrl, family = family)  # for scalar on function regression, the time formula is always ~bols(1)
}
