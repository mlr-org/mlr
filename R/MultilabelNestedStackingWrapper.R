...
makeMultilabelNestedStackingWrapper = function(learner, order = "random", cv.folds = 2) {
  ...
  x = makeHomogeneousEnsemble(id, learner$type, learner, packs, learner.subclass = "MultilabelNestedStackingWrapper", model.subclass = "MultilabelNestedStackingModel")
  x$type = "multilabel"
  x$order = order
  x$cv.folds = cv.folds
  return(x)
...
trainLearner.MultilabelNestedStackingWrapper = function(.learner, .task, .subset, .weights = NULL,...) {
  if (.learner$order[1] == "random") {
    order=sample(1:length(.task$task.desc$target))
  } else {
    order=.learner$order
  }
  if (length(order) != length(.task$task.desc$target)) stopf("Length order does not match number of targets!")
  ...
  for (i in 1:length(targets)) {
    ...
    if (i >= 2) {
      ...
      data2 = data.frame(dataNST, data[tnprevious])
      innertask = makeClassifTask(id = tnprevious, data = data2, target = tnprevious)
      r = resample(.learner$next.learner, innertask, makeResampleDesc("CV", iters = .learner$cv.folds), show.info = FALSE)
      predlabel = as.numeric(as.logical(r$pred$data[order(r$pred$data$id), ]$response))
      data2 = data.frame(dataNST, data[c(tnprevious, tn)])
