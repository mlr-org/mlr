...
makeMultilabelClassifierChainsWrapper = function(learner, order = "random") {
  ...
  x = makeHomogeneousEnsemble(id, learner$type, learner, packs,
    learner.subclass = "MultilabelClassifierChainsWrapper", model.subclass = "MultilabelClassifierChainsModel")
  x$type = "multilabel"
  x$order = order
  return(x)
}
...
trainLearner.MultilabelClassifierChainsWrapper = function(.learner, .task, .subset, .weights = NULL,...){
  ...
  if (length(order) != length(.task$task.desc$target)) stopf("order length does not match number of targets!")  
