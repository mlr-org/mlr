...
makeRLearner.classif.rda = function() {
  makeRLearnerClassif(
    ...
    par.set = makeParamSet(
      ...
      makeDiscreteLearnerParam(id = "schedule", default = 1L, values = 1:2, requires = quote(simAnn==FALSE)),
      makeNumericLearnerParam(id = "T.start", default = 0.1, lower = 0, requires = quote(simAnn==TRUE)),
      makeNumericLearnerParam(id = "halflife", default = 0.1, lower = 0, requires = quote(simAnn==TRUE || schedule==1)),
    ...
    mlr.defaults = list(estimate.error = FALSE),
