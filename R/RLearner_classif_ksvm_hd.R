#' @export
makeRLearner.classif.ksvm.hd = function() {
  makeRLearnerClassif(
    cl = "classif.ksvm.hd",
    package = "kernlab",
...
#' @export
trainLearner.classif.ksvm.hd = function(.learner, .task, .subset, .weights = NULL, degree, offset, scale, sigma, order, length, lambda, normalized,  ...) {
...
#     }
  ...
  if (base::length(kpar) > 0L)
    kernlab::ksvm(x = d$data, y = d$target, kpar = kpar, prob.model = pm, ...)
  else
    kernlab::ksvm(x = d$data, y = d$target, prob.model = pm, ...)
}

#' @export
predictLearner.classif.ksvm.hd = function(.learner, .model, .newdata, ...) {
