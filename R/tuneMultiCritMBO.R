tuneMultiCritMBO = function(learner, task, resampling, measures, par.set, control,
  ...
  x = lapply(or$pareto.set, trafoValue, par.set = par.set)
  y = setNames(or$y, opt.path$y.names)
  # we take the points that mbo proposes and its estimated ys
  # FIXME: threshold
  if (!control$mbo.keep.result)
    or = NULL
  res = makeTuneMultiCritResult(learner, control, ind, x, y, NULL, opt.path, mbo.result = or)
  res
}
