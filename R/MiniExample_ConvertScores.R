...
# nu 0.5 per default
svm.model0.5 <- svm(data[, 1:4], y = NULL, type='one-classification', nu = 0.5, kernel="radial", scale=TRUE)
svm.pred0.5 <- predict(svm.model0.5, data[,1:4], decision.values = TRUE)
...
# nu 0.09
svm.model0.09 <- svm(data[, 1:4], y = NULL, type='one-classification', nu = 0.09, kernel="radial", scale=TRUE)
svm.pred0.09 <- predict(svm.model0.09, data[,1:4], decision.values = TRUE)
...
svm.model0.05 <- svm(data[, 1:4], y = NULL, type='one-classification', nu = 0.05, kernel="radial", scale=TRUE)
svm.pred0.05 <- predict(svm.model0.05, data[,1:4], decision.values = TRUE)
