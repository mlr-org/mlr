...
# @export
# @rdname TuneControl
makeTuneMultiCritControlMBO = function(same.resampling.instance = TRUE, impute.val = NULL,
  ...
  if(!is.null(budget) && !is.null(mbo.design) && nrow(mbo.design) > budget)
  ...
  else if (!is.null(budget)) { # if at least budget is not null, while, mbo.design is not sure 
    ...
    mbo.control = setMBOControlTermination(mbo.control, max.evals = budget) # append properties to mbo.control
  }
